...
| `ACADEMY_BALE_WORKERS` | `3` | تعداد تردهای پردازش پیام بله — روی هاست کند یا با کاربر زیاد بیشترش کنید (ترتیب پیام‌های هر کاربر حفظ می‌شود) |
| `ACADEMY_ASSET_BUNDLE` | `1` | ادغام CSS/JS در یک فایل (`static/gen/`) برای کم‌کردن تعداد درخواست‌ها؛ `0` به فایل‌های جدا برمی‌گردد |

> بستهٔ **اختیاری** `brotli` فشرده‌سازی استاتیک را حدود ۱۵٪ بهتر از gzip می‌کند
> (`pip install brotli`). عمداً در `requirements.txt` نیست؛ اگر نصب نباشد gzip
> جای آن را می‌گیرد و هیچ چیز نمی‌شکند.
